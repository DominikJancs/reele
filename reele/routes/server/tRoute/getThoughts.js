...
async function getthought(req, res) {
    ...
    if (result) {
        ...
        else res.status(404).send("Something went wrong! 0").json();
    }
    else res.status(404).send("Something went wrong! 1").json();
}
...
async function createthought(req, res, thoughts) {
    ...
    for (let i = 0; i < thoughts.length; i++) {
        ...
        if (result) {
            ...
            else res.status(404).send("Something went wrong 2!").json();
        }
        else res.status(404).send("Something went wrong! 3").json();
    }
    return thoughtsData;
}
